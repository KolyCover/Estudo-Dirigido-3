...
void gerador_de_dezenas_aleatorias(){
    srand(time(NULL));
        ...
        for(int j = 0; j < qtddezenas; j++){
            dezenas.push_back((rand()% limite)+ somador);
        }

        removedor_de_repeticao();

        cout << "\n\nAs dezenas geradas para você foram:\n";
        for(int nuns:dezenas){
            cout << nuns << " ";
        }
...
void removedor_de_repeticao(){
     ...
     sort(begin(dezenas), end(dezenas));

    for(int i = 0, j = 1, k = 2; i < qtddezenas; i++, j++, k++){
            while(dezenas[i] == dezenas[j] || dezenas [j] == dezenas [k]){
                if(dezenas[i] == dezenas[j]){
                    dezenas[i] = (rand() % limite) + somador;
                }

                else if(dezenas [j] == dezenas [k]){
                    dezenas[j] = (rand() % limite) + somador;
                }
